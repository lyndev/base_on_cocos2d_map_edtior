...
function CUIManager:OpenUI(msgBuf)
    local msg = DeserializeFromStr(msgBuf)
    OpenUI(self, msg)
end
...
-- 备    注:
--]]
function CUIManager:MessageProc(nMsgID, pData, nLen)
    -- 处理打开/关闭
    if MSGID.CC_OPEN_UI == nMsgID then
        CUIManager:GetInstance():OpenUI(pData)
        return
    elseif MSGID.CC_CLOSE_UI == nMsgID then
        CUIManager:GetInstance():CloseUI(pData)
        return
    end

    -- 分发
    for _, ui in pairs(self.m_UIList) do
        if ui.MessageProc ~= nil then
            ui:MessageProc(nMsgID, pData, nLen)
        end
    end
end
