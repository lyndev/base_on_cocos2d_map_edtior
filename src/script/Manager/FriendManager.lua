...
-- 客户端请求删除好友
function CFriendManager:ReqDeleteFriend( playerID )
    if not playerID or playerID == '' then
        return
    end
    local msg = {}
    msg.id  = tostring(playerID)
    SendMsgToServer(MSGID.CS_FRIEND_REQREMOVEFRIEND, msg)
end

-- 同意/拒绝
function CFriendManager:ReqAgreeFriend(playerID, bAgree)
    if not playerID or playerID == "" then
        return
    end
    local msg = {}
...
-- 发送聊天内容
function CFriendManager:ReqFriendChat(reciverPlayerID, content)
    if not reciverPlayerID or reciverPlayerID == "" then
        Notice("接收者不能为空")
        return
    end
    ...
    msg.receiverUid = reciverPlayerID
    msg.text = content
...
function CFriendManager:SetChatReadWithFriend(playerID)
    local _chats = self.m_chatList[playerID]
    if _chats then
...
-- 客户端申请加好友
function CFriendManager:ReqApplyFriend(playerID)
    if not playerID or playerID == '' then
        return
    end
    local msg = {}
...
-- 获取聊天
function CFriendManager:GetChat(playerID)
    return self.m_chatList[playerID] or {}
end
...
-- 获取一个好友的信息
function CFriendManager:GetFriend(playerID)
    if playerID then
        log_info(LOG_FILE_NAME, "获取好友信息的id:%s", playerID)
        return self.m_tFriendList[playerID]
    else
        log_error(LOG_FILE_NAME, "获取好友信息失败,playerID为空")
    end
end

function CFriendManager:IsOnline(playerID)
    local _friend = self:GetFriend(playerID)
    if _friend then
    ...
    for id, chats in pairs(self.m_chatList) do
        for i,chat in ipairs(chats) do
            if not chat.bRead then
                table.insert(_notReadChatPlayerInfo, {playerID = id, sendtime = chat.sendtime})
                break
            end
        end
    end
...
function CFriendManager:IsFirend( playerID )
    return self.m_tFriendList[playerID]
end
