...
function CFightBase:AddPlayer(playerID, player)
    if player then
        self.m_fightPlayers[playerID] = player
    else
        log_error(LOG_FILE_NAME, "添加战斗玩家失败,玩家对象为空")
    end
end

function CFightBase:RemovePlayer(playerID)
    if self.m_fightPlayers[playerID] then
        self.m_fightPlayers[playerID] = nil
    else
...
function CFightBase:GetPlayer(playerID)
    return self.m_fightPlayers[playerID]
end
...
function CFightBase:Destroy()
    ...
    CMsgRegister:UnRegListenerHandler(MSGID.SC_DAER_LEAVEROOMACK, "CFightBase_SC_DAER_LEAVEROOMACK")
    CMsgRegister:UnRegListenerHandler(MSGID.SC_PLAYER_LEAVECUSTOMROOMACK, "CFightBase_SC_PLAYER_LEAVECUSTOMROOMACK")
