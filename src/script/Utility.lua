...
function AddEventListener( node, bSwallow, onBeganCallback, onMovedCallback, onEndedCallback)
    ...
    local function onTouchEnded( touch, event )
        ...
        if cc.rectContainsPoint(_rect, _location) then
            if onEndedCallback and type(onEndedCallback) == 'function' then
                onEndedCallback(node, touch:getLocation())
            end
        end        
    end
    ...
    -- 吞没事件 
    listener:setSwallowTouches(bSwallow or false)
...
-- 备    注:
--]]
function OpenUI( name, folder, data)
    if folder == '' or nil then
        folder = 'UILogic'
    end
    local msg = data or {}
    msg.name = name
    msg.FolderName = 'UILogic'
    
    local buffer = SerializeToStr( msg )
    SendLocalMsg( MSGID.CC_OPEN_UI, buffer, #buffer )  
end
...
function CloseUI( name )
    SendLocalMsg(MSGID.CC_CLOSE_UI, name, #name)
end
...
function FormatMoney(num)
    ...
    --     local _cutNum = #nums - 4
    --     local _afterNum = ''
    --     for i=1, _cutNum do
    --         _afterNum = _afterNum .. nums[i]
    --     end
    --     return _afterNum
    -- elseif num >= 100000000 then
    --     local _cutNum = #nums - 8
    --     local _afterNum = ''
    --     for i=1, _cutNum do
    --         _afterNum = _afterNum .. nums[i]
    --     end
...
function AddButtonEvent(btn, callback, args)
    assert(btn)
    assert(callback)
...
function GrayNode(node)
    if node then
        ...
        for k, v in pairs(_allChidren) do
            v:setColor(cc.c3b(140, 140, 140))
        end
        node:setColor(cc.c3b(140, 140, 140))
    end
end
...
function GetDateMs(y, m, d)
    if y and m and d then
        return os.time({day = d, month = m, year = y, hour=0, minute=0, second=0}) * 1000
    else
...
function SetOneNoticeText(_lineTxtItem, noticeInfo, type)
...
function CreateOneNoticeText(noticeInfo, type)
    local _lineTxtItem = cc.CSLoader:createNode("ui_csb/ui_template_fonts.csb")
    if _lineTxtItem then
        _lineTxtItem:setContentSize(cc.size(_lineTxtItem:getContentSize().width, 30))
        --_lineTxtItem:setAnchorPoint(cc.p(0, 0.5))
        return SetOneNoticeText(_lineTxtItem, noticeInfo, type)
    end
end
