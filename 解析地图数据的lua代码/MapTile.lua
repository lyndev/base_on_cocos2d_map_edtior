...
function CMapTile:Init(id)
	...
	self.m_hurtEffectId     = Q_MapElement.GetTempData(id, "q_hurt_effect") or 0
	self.m_destroyEffectId  = Q_MapElement.GetTempData(id, "q_destroy_effect") or 0
...
-- 得到受伤特效
function CMapTile:GetHurtEffect()
	return self.m_hurtEffectId 
end

-- 得到爆炸特效
function CMapTile:GetDestroyEffect()
	return self.m_destroyEffectId 
end

-- 获取地块图片的大小
function CMapTile:GetTitleSize()
	local width = 0
	local height = 0
	if self.m_sprite then
		width = self.m_sprite:getContentSize().width
		height = self.m_sprite:getContentSize().height
	end
	return width, height
end

-- 保存创建的受伤特效ID
function CMapTile:SetHurtEffectId(instanceId)
	self.m_hurtEffectInstanceId = instanceId or 0
end

-- 获取创建的受伤特效ID
function CMapTile:GetHurtEffectId()
	return self.m_hurtEffectInstanceId
end
...
function CMapTile:Destroy()
	...
	if self.m_shadow then
	    self.m_shadow:removeFromParent()
	end
end