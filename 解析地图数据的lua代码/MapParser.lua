...
require("script.Map.MapTile")

CMapParser = class('CMapParser', CMapBase)
local DRAW = false

function CMapParser:New()
    local o = CMapBase:New()
    o.m_mapTiles = {}
    o.m_mapCollisionFlag = {}
    setmetatable(o, CMapParser)
    return o
end

function CMapParser:Init(id)
	CMapParser.super.Init(self)
    -- 加载受损显示的元素图集
...
function CMapParser:LoadMapConfig(id)
...
-- 读取即将使用的地图图片
function CMapParser:LoadMapWillUseImagePath(ids)
...
function CMapParser:DraMapElements()
...
function CMapParser:DrawMapBackground()
...
function CMapParser:AddElement(root, tile, id)
...
function CMapParser:RemoveElment(tile)
...
function CMapParser:BreakElement(tile)
	local _tileObj= self:GetTileObj(tile)
	if _tileObj then
		_tileObj:ShowBreak()
	end
...
function CMapParser:GetTileObj(tile)
...
function CMapParser:AddElementCollision(tile, tileObj)
...
function CMapParser:RemoveElementCollision(tile, tileObj)
	if tileObj then
		local _size = tileObj:GetCollisionSize()
		local _isBulletBlock = tileObj:IsBulletCollision()
		local _camp = tileObj:GetCamp()
...
function CMapParser:GetCollisionByTile(tile)
...
function CMapParser:RemoveAllElement()
...
function CMapParser:Destroy()
	for i,v in ipairs(self.m_plists) do
    	CPlistCache:GetInstance():ReleasePlist(v)
    end
	self:RemoveAllElement()
	CMapParser.super.Destroy(self)
    CMapParser._instance = nil
end